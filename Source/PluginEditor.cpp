...
void LookAndFeel::drawRotarySlider(juce::Graphics & g,
  ...
  g.setColour(enabled ? Colour(97 u, 18 u, 167 u) : Colours::darkgrey);
  g.fillEllipse(bounds);

  g.setColour(enabled ? Colour(255 u, 154 u, 1 u) : Colours::grey);
...
void LookAndFeel::drawToggleButton(juce::Graphics & g,
  ...
  if (auto * pb = dynamic_cast < PowerButton * > ( & toggleButton)) {
    ...
    auto color = toggleButton.getToggleState() ? Colours::dimgrey : Colour(0 u, 172 u, 1 u);
...
void RotarySliderWithLabels::paint(juce::Graphics & g) {
  ...
  auto radius = sliderBounds.getWidth() * 0.5 f;

  g.setColour(Colour(0 u, 172 u, 1 u));
  ...
  for (int i = 0; i < numChoices; ++i) {
    ...
    auto c = center.getPointOnCircumference(radius + getTextHeight() * 0.5 f + 1, ang);
...
void ResponseCurveComponent::paint(juce::Graphics & g) {
  ...
  if (shouldShowFFTAnalysis) {
    ...
    g.setColour(Colour(97 u, 18 u, 167 u)); //purple-
    g.strokePath(leftChannelFFTPath, PathStrokeType(1. f));

    auto rightChannelFFTPath = rightPathProducer.getPath();
    rightChannelFFTPath.applyTransform(AffineTransform().translation(responseArea.getX(), responseArea.getY()));

    g.setColour(Colour(215 u, 201 u, 134 u));
...
void ResponseCurveComponent::drawBackgroundGrid(juce::Graphics & g) {
  ...
  for (auto gDb: gain) {
    ...
    g.setColour(gDb == 0. f ? Colour(0 u, 172 u, 1 u) : Colours::darkgrey);
...
void ResponseCurveComponent::drawTextLabels(juce::Graphics & g) {
  ...
  for (auto gDb: gain) {
    ...
    g.setColour(gDb == 0. f ? Colour(0 u, 172 u, 1 u) : Colours::lightgrey);
...
void SimpleEQAudioProcessorEditor::paint(juce::Graphics & g) {
  ...
  curve.lineTo(center.x - titleWidth * 0.45 f, 32);
  ...
  g.setColour(Colour(97 u, 18 u, 167 u));
  g.fillPath(curve);
  ...
  g.setColour(Colour(255 u, 154 u, 1 u));
